...
switch exptName
    ...
    case 'qtask_trc'
        ...
        truncPt = 16; % time in s up to which survival curves are computed
    case 'qtask_nodra'
        loadFx = @loadData_nodra;
        truncPt = 16; % time in s up to which survival curves are computed
        runTest.fieldsForTxtOutput = {'AUC', 'AUC_after1s','FastQuits','TotalEarnings','BlockDuration'};
    case 'qtask_ftd'
        loadFx = @loadData_ftd;
        truncPt = 16;
        blockSec = 420;
...
grpIDs = {grpData(:).grpID}; %orig

%grpdates = {grpData(:).sessionTime}; %this variable has date and time of study

%grpIDs = char(num2str(100*ones(length(grpData),1)));

grpNames = unique(grpIDs);

%grpNames = char(unique(grpIDs));
...
for g = 1:nGrps
    ...
    for s = 1:gN
        ...
        date = grpData(sIdx).sessionTime %NEW LINE TO TRY

        alldates.(gName){s,1} = date; %NEW LINE TO TRY
        ...
        for b = 1:nBks
            ...
            bkTrials = grpData(sIdx).trialData(cell2mat({grpData(sIdx).trialData.bkIdx})==b);
            %bkTrials = grpData(sIdx).trialData(b);
            
            %mack added to format data
            quit = num2cell(strcmp({bkTrials.trialResult}, 'quit'));
            win = num2cell(strcmp({bkTrials.trialResult}, 'win'));
            [bkTrials.outcomeWin] = win{:};
            [bkTrials.outcomeQuit] = quit{:};
            ...
            if runTest.FastQuits
                ...
                isFastQuit = cell2mat({bkTrials.outcomeQuit}) & cell2mat({bkTrials.latency}) < 0.1;
                results.FastQuits.(gName)(s,b) = sum(isFastQuit);
            end

            % calculate the total earnings in this block
            if runTest.TotalEarnings
                totalEarned = cell2mat({bkTrials.totalEarned});
                payoff = cell2mat({bkTrials.payoff});
                priorPay = totalEarned(1) - payoff(1); % earnings _before_ the first trial
                finalPay = totalEarned(end);
            ...
            if runTest.BlockDuration
                outcomeTime = cell2mat({bkTrials.outcomeTime});
                results.BlockDuration.(gName)(s,b) = outcomeTime(end);
            end
        ...
        if runTest.RunningWTW
            ...
            %[sData.outcomeWin] = deal(ismember({sData.trialResult},'win'));
            %[sData.outcomeQuit] = deal(ismember({sData.trialResult},'quit'));

tmp1=ismember({sData.trialResult},'win');
tmp2=ismember({sData.trialResult},'quit');
for i=1:length(sData);sData(i).outcomeWin=tmp1(i);sData(i).outcomeQuit=tmp2(i);end;
     	    
	%results_runningWTW.(gName)(s,:) = runningWtw(sData,blockSec,truncPt)';
	results_runningWTW.(gName)(s,1:420*length(sData)) = runningWtw(sData,blockSec,truncPt)';
   %this hack works but: will blockSec always be 420? why is this program expecting same trial number (sData size) for all subjects but we have different sizes?  what are consequences of having empty data in the results_runningWTW.(gname) for the subjects with less-than-maximum-trial-number?
            %results_runningWTW.(gName)(s,:) = runningWtw(sData,blockSec,truncPt)';%syntax on left uses the gName variable as a field in the structure; the s is the subject number, the column ends up 420(blocksec)xthenumberoftrials in sData which varies by subject (eg, here 123 then 87). 
...
if isfield(runTest,'fieldsForTxtOutput') && ~isempty(runTest.fieldsForTxtOutput)
    txtOutput(exptName,runTest.fieldsForTxtOutput,allIDs,alldates, results);
end
